...
test('real sign-in, pipeline list and canvas edit preserve stored metadata', async ({ page, context }, testInfo) => {
  ...
  try {
    const listResponse = page.waitForResponse(response =>
      new URL(response.url()).pathname === '/api/pipelines' && response.request().method() === 'GET');
    await page.goto('/pipelines');
    const listed = await listResponse;
    expect(listed.ok()).toBeTruthy();
    const list = await listed.json();
    expect(list.rows.some((row: any) => row.id === fixture.rowId)).toBe(true);
    ...
    const originalResponse = page.waitForResponse(response =>
      new URL(response.url()).pathname === `/api/pipelines/${fixture.rowId}` && response.request().method() === 'GET');
