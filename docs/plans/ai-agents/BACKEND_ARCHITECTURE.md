...
The first implementation supports the `workflow` engine, immutable agent versions, a bounded model/tool loop, operator-configured Ollama, HTTP and remote MCP tools, human approval for mutations, and run-scoped memory. Reject agent nodes in `stream-direct`, `spark-sql`, and `flink-sql` until separate semantics are designed. LangGraph and persistent cross-run memory are [deferred options](README.md#deferred-options), outside the committed B1–B5/F1–F5 scope. Revisit LangGraph only for a concrete need to reuse an existing Python agent while preserving the same execution, tool, approval, budget and cancellation contracts; it is not an automatic next milestone. No second control plane, scheduler, generic plugin platform, or new backend language is needed for the native runtime.
